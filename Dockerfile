...
ADD --keep-git-dir=true --link https://gitlab.com/flightgear/fgmeta.git /flightgear/script/fgmeta

WORKDIR /flightgear/script/fgmeta

RUN git switch $(git remote show origin | sed -n '/HEAD branch/s/.*: //p')

RUN git submodule deinit --force --all

RUN git pull --no-recurse-submodules origin $(git remote show origin | sed -n '/HEAD branch/s/.*: //p')
...
ADD --link https://gitlab.com/flightgear/simgear.git#release/${branch_end} /dev/null

ADD --link https://gitlab.com/flightgear/flightgear.git#release/${branch_end} /dev/null

ADD --link https://gitlab.com/flightgear/fgdata.git#release/${branch_end} /dev/null

ADD --link https://gitlab.com/flightgear/openscenegraph.git#release/2024-build /dev/null
...
"if (float($4) < 0):
    ...
    ./terrasync.py -t ~/.fgfs/TerraSync/ --top $ROUNDED_TOP --bottom $ROUNDED_BOTTOM --left $ROUNDED_LEFT --right $ROUNDED_RIGHT --report -u https://us1mirror.flightgear.org/terrasync/ws3/ --only-subdir NavData
    ./terrasync.py -t ~/.fgfs/TerraSync/ --top $ROUNDED_TOP --bottom $ROUNDED_BOTTOM --left $ROUNDED_LEFT --right $ROUNDED_RIGHT --report -u https://us1mirror.flightgear.org/terrasync/ws3/ --only-subdir Objects
    ./terrasync.py -t ~/.fgfs/TerraSync/ --top $ROUNDED_TOP --bottom $ROUNDED_BOTTOM --left $ROUNDED_LEFT --right $ROUNDED_RIGHT --report -u https://us1mirror.flightgear.org/terrasync/ws3/ --only-subdir Terrain
    ./terrasync.py -t ~/.fgfs/TerraSync/ --top $ROUNDED_TOP --bottom $ROUNDED_BOTTOM --left $ROUNDED_LEFT --right $ROUNDED_RIGHT --report -u https://us1mirror.flightgear.org/terrasync/ws3/ --only-subdir vpb
